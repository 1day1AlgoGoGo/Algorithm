...
public class 상어초등학교_PQ {
    ...
    private static void solve() {
        Queue<Seat> queue = new PriorityQueue<>();
        for (int student : students) {
            findBestSeat(queue, student);
            queue.clear();
        }
    }
    ...
    private static void findBestSeat(Queue<Seat> queue, int studentNumber) {
        for (int r = 0; r < N; r++) {
            for (int c = 0; c < N; c++) {
                if (grid[r][c] != 0) {
                    continue;
                ...
                queue.add(setSeat(r, c, studentNumber));
            }
        }

        // 우선순위큐로 저장했기 때문에 Comparable 메소드로 비교되어 나옴
        Seat bestSeat = queue.poll();
        // 해당 자리에 현재 학생 번호 저장
        grid[bestSeat.getR()][bestSeat.getC()] = studentNumber;
        list[studentNumber] = bestSeat;
    }
